...
> R: (escribí el total esperado)2800
...
> R:Inicializa la varibale para que no quede con basura
...
> R:linea 24, codigo suma el precio de todos los productos del carrito sin tener en cuenta la cantidad de cada tipo de producto

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Multiplicar por la cantidad de cada producto
...
> R:falla, devuelve esto:[FAIL] carrito_contar(&c) => esperado 4, obtenido 1751344461  (linea 69)
...
> R:Si, las lineas 29 y 30 no se ejecutan porque la funcion carrito_descuento no se llama en test_unitarios.c
...
> R:El test unitario prueba una funcion aislada y el de integracion prueba varias funciones juntas. El bug de carrito_total lo detecto primero el unitario
...
> R:Porque C no controla los limites del array y puede sobrescribir memoria. En Python o Java se detecta el acceso fuera de rango
...
> R:Permite probar automaticamente distintos casos y comparar el resultado esperado con el obtenido
...
> R:Porque con cantidad 1, multiplicar o no por la cantidad da el mismo resultado. Con cantidad 2 el error se hace visible
